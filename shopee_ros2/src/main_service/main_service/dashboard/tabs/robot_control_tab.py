...
class RobotControlTab(BaseTab, Ui_RobotControlTab):
    ...
    def _setup_table_columns(self):
        ...
        self.mission_queue_table.setColumnWidth(5, 100)  # 예상 완료
        self.mission_queue_table.setColumnWidth(6, 80)   # 배터리

        # 마지막 컬럼(진행률)은 남은 공간을 모두 차지하도록 설정
    ...
    def _update_mission_queue(self, robots: List[Dict[str, Any]], orders: Dict[str, Any]):
        ...
        for row, task in enumerate(tasks):
            ...
            # 컬럼 5: 예상 완료
            self.mission_queue_table.setItem(row, 5, QTableWidgetItem('-'))
            ...
            self.mission_queue_table.setItem(row, 6, battery_item)

            # 컬럼 7: 진행률
            self.mission_queue_table.setItem(row, 7, QTableWidgetItem('-'))

...
except ImportError:  # pragma: no cover - 장비가 없는 환경에서도 노드를 기동할 수 있도록 처리
    MyCobot280 = None
...
class JetCobotArm:
    ...
    def _connect(self) -> None:
        ...
        except Exception as exc:  # pragma: no cover - 하드웨어 예외 처리
            self._node.get_logger().error(
                f"[{self._arm_name}] JetCobot 연결에 실패했습니다: {exc}")
            self._robot = None

    def handle_twist(self, message: TwistStamped) -> None:
        """Packee Arm 컨트롤러가 발행한 속도 명령을 적분해 좌표 이동을 수행한다."""
        now_stamp = Time.from_msg(message.header.stamp) if message.header.stamp.sec != 0 else self._node.get_clock().now()
        ...
        self._pose["yaw_deg"] += math.degrees(message.twist.angular.z) * dt
    ...
    def handle_gripper(self, message: Float32) -> None:
        ...
        try:
            ...
            self._node.get_logger().error(
                f"[{self._arm_name}] 그리퍼 명령 전송 실패: {exc}")
...
class JetCobotBridge(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("right_serial_port", "")
        ...
        self.declare_parameter("default_pose_cart_view", [0.16, 0.0, 0.18, 0.0])
        self.declare_parameter("default_pose_standby", [0.10, 0.0, 0.14, 0.0])
        ...
        self._left_arm = JetCobotArm(
          ...
          move_speed,
          self.get_parameter("default_pose_cart_view").value,
          workspace,
          fallback_dt,
          open_value,
          close_value)
        self._right_arm = JetCobotArm(
          ...
          move_speed,
          self.get_parameter("default_pose_standby").value,
          workspace,
          fallback_dt,
          open_value,
          close_value)

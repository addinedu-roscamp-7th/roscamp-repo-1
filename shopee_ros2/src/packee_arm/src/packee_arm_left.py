...
class PackeeArmController(Node):
    def __init__(self):
        ...
        ## 서비스 서버
        self.move_to_pose_server = self.create_service(ArmMoveToPose, "/packee2/arm/move_to_pose", self.move_to_arm)
    ...
    def move_to_arm(self, request: ArmMoveToPose.Request,response: ArmMoveToPose.Response):
        ...
        try:
            self.get_logger().info("packee2 Arm 자세변경을 수락했습니다.")
            ...
            self.get_logger().info("packee2 Arm 자세변경 완료했습니다.")

            response.success = True
            response.message = "packee2 Arm 자세변경 완료"

        except Exception as e:
            self.get_logger().info("packee2 Arm 자세변경을 실패했습니다.")

            response.success = False
            response.message = "packee2 Arm 자세변경 실패"
    ...
    def pickup_product(self, request:ArmPickProduct.Request, response:ArmPickProduct.Response):
        ...
        try:
            ...
            while not complete_flag:
                ...
                self.get_logger().info(f"packee2 Arm을 x={pose[0]}, y={pose[1]}, z={pose[2]}, rx={pose[3]}, ry={pose[4]}, rz={pose[5]} 만큼 이동합니다.")
            ...
            self.get_logger().error("packee2 상품 픽업을 실패했습니다.")

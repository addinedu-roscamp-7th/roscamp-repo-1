...
def parse_args() -> argparse.Namespace:
    """명령행 인자를 파싱합니다."""
    parser = argparse.ArgumentParser(description='SC_02_4 상품 담기 시나리오 실행기')
    ...
    parser.add_argument('--bbox-number', type=int, default=1, help='선택할 박스 번호 (기본값: 1)')
    parser.add_argument('--product-id', type=int, help='선택할 상품 ID (없으면 장바구니 첫 상품 사용)')
    parser.add_argument(
        '--cart-items',
...
def main() -> None:
    ...
    asyncio.run(
        run_sc_02_4_product_selection(
            host=args.host,
            port=args.port,
            user_id=args.user_id,
            password=args.password,
            cart_items=cart_items,
            bbox_number=args.bbox_number,

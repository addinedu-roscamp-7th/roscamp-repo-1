...
from src.zlac_driver import ZLACDriver
...
class PickeeMobileWonhoRobot(Node):
    """
    ROS 2 Node that uses the ZLACDriver module to control the robot.
    """
    def __init__(self):
        super().__init__('pickee_mobile_wonho_robot')
        self.is_initialized = False # 초기화 성공 여부 플래그

        self.get_logger().info('Initializing Pickee Mobile Wonho Robot Node...')
        ...
        self.get_logger().info('Pickee Mobile Wonho Robot has been started successfully.')

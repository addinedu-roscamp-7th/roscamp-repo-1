...
`pickee_vision` 패키지는 `Pickee` 로봇의 시각 인지 기능을 담당합니다. 이 패키지는 두 가지 주요 기능을 수행합니다: (1) 선반의 상품을 탐지하고 카트의 상태를 확인하는 일반적인 분석 기능과 (2) 특정 상품을 정밀하게 집기 위한 시각 서보(Visual Servoing) 기반의 고정밀 피킹 시퀀스.
...
`pickee_vision`의 모든 기능을 통합하여 실행하는 메인 노드입니다. 여러 카메라로부터 영상을 입력받아 상품 탐지, 카트 상태 분석, 시각 서보 등 다양한 컴퓨터 비전 작업을 수행하고 다른 노드와 통신합니다.

## 핵심 기능

### 1. 선반 및 카트 분석

독립적으로 호출될 수 있는 서비스들을 통해 선반의 상품 목록을 분석하거나 카트의 상태를 확인합니다.

-   **상품 탐지**: `YoloDetector`를 사용하여 선반 위 상품들의 위치와 종류를 탐지합니다. (`/pickee/vision/detect_products`)
-   **카트 내 상품 확인**: 카트 안에 특정 상품이 있는지 확인합니다. (`/pickee/vision/check_product_in_cart`)
-   **카트 존재 여부 확인**: `CnnClassifier`를 사용하여 로봇 앞에 빈 카트가 있는지 확인합니다. (`/pickee/vision/check_cart_presence`)
...
고정밀 피킹은 `pickee_main`, `pickee_vision`, `pickee_arm` 세 노드 간의 긴밀한 협력을 통해 이루어집니다. `pickee_main`이 전체 과정을 조율하는 오케스트레이터 역할을 합니다.
...
+--------------+      +--------------------------+      +------------------+
       ...
       | (e.g. /pick_product_gg)   |   (e.g. /move_start)      |
       |                           |                           |
       |                           | <----(3) Arm is Ready ---- |
       |                           |   (e.g. /is_moving)       |
       |                           |                           |
...
1.  **시퀀스 시작**: `pickee_main` 노드가 `pickee_vision`의 `/pickee/arm/pick_product_gg` 서비스를 호출하여 특정 상품에 대한 피킹 시퀀스를 시작합니다.
2.  **팔 초기 이동**: `pickee_vision`은 `pickee_arm`의 `/move_start` 서비스를 호출하여 팔을 선반 보기 자세로 움직이도록 명령합니다.
3.  **팔 준비 완료 신호**: `pickee_arm`은 이동이 완료되면 `/is_moving` 토픽으로 `True`를 게시하여 `pickee_vision`에 준비되었음을 알립니다.
4.  **시각 서보 루프**: `pickee_vision`은 `PoseCNN`을 이용한 시각 서보 루프를 시작합니다. 현재 이미지와 팔의 실제 좌표(`real_pose`)를 바탕으로 오차를 계산하고, PID 제어기를 통해 보정된 목표 좌표를 `/move_servo` 토픽으로 계속해서 `pickee_arm`에 발행합니다.
...
-   **`/pickee/arm/move_servo` (`Pose6D`)**: 시각 서보 중 팔이 움직여야 할 보정된 6D 좌표를 게시합니다.
...
-   **`PoseCNN`**: 특정 상품(예: 생선, 이클립스)의 6D Pose를 추정하여 시각 서보에 사용되는 CNN 모델.

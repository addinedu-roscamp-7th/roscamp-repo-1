#include "pickee_mobile_wonho/custom_planner_simulation.hpp"
#include <pluginlib/class_list_macros.hpp>

namespace custom_planner_simulation
{

    CustomPlannerSimulation::CustomPlannerSimulation()
    {
    ...
    void CustomPlannerSimulation::configure(
        ...
        std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
    {
        ...
        if (node)
        {
            ...
            RCLCPP_INFO(logger_, "CustomPlannerSimulation이 구성되었습니다.");

            // 초기화 서비스 생성
            reset_service_ =  node->create_service<std_srvs::srv::Trigger>(
                "custom_planner/reset",
                std::bind(&CustomPlannerSimulation::ResetCallback, this, std::placeholders::_1, std::placeholders::_2)
            );
    ...
    void CustomPlannerSimulation::ResetCallback(
        const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
        std::shared_ptr<std_srvs::srv::Trigger::Response> response)
    {
        (void)request;
        
        RCLCPP_INFO(logger_, "CustomPlannerSimulation 초기화 시작...");
    // 초기화
    is_after_plan = false; 
    global_path.poses.clear();
        ...
        if (node) {
            if (is_narrow_passage) {
                narrow_passage_timer_ = node->create_wall_timer(
                    ...
                    std::bind(&CustomPlannerSimulation::NarrowPassageTimerCallback, this)
                );
                // 좁은 문 통과후 다시 주행 시작
        ...
        response->message = "CustomPlannerSimulation 초기화 완료";
        RCLCPP_INFO(logger_, "CustomPlannerSimulation이 초기화되었습니다.");
    }

    void CustomPlannerSimulation::NarrowPassageTimerCallback()
    {
        if (!is_narrow_passage) {
        ...
        double current_y = current_pose.pose.pose.position.x;
        double current_x = current_pose.pose.pose.position.y;
    ...
    void CustomPlannerSimulation::cleanup()
    {
        RCLCPP_INFO(logger_, "CustomPlannerSimulation이 cleanup되었습니다.");
    }

    void CustomPlannerSimulation::activate()
    {
        RCLCPP_INFO(logger_, "CustomPlannerSimulation이 activate되었습니다.");
    }

    void CustomPlannerSimulation::deactivate()
    {
        RCLCPP_INFO(logger_, "CustomPlannerSimulation이 deactivate되었습니다.");
    }

    // 1. 출발지 좌표 찾기 함수 & 2. 도착지 좌표 찾기 함수 통합
    std::optional<WaypointResult> CustomPlannerSimulation::GetStartEndWaypoint(
    ...
    std::optional<std::map<std::string, geometry_msgs::msg::PoseStamped>> CustomPlannerSimulation::GetBetweenWaypoints(
        const geometry_msgs::msg::PoseStamped &start, 
        const geometry_msgs::msg::PoseStamped &goal,
    ...
    void CustomPlannerSimulation::MakePathByBetweenWaypoints(
    ...
    void CustomPlannerSimulation::AddDetailedPath()
    {
    ...
    // 진행 방향 얻기 함수
    void CustomPlannerSimulation::AddOrientationToPath()
    {
    ...
    std::optional<bool> CustomPlannerSimulation::GetProgressedDirection(const std::string &current_wp_key, const std::string &before_wp_key, const geometry_msgs::msg::PoseStamped &start)
    {
    ...
    void CustomPlannerSimulation::CheckNarrowPassage()
    {
        ...
        bool has_x1y1 = false;
        for (const auto &pose : global_path.poses) {
            if (std::abs(pose.pose.position.x - y2_) < 1e-6 && std::abs(pose.pose.position.y - x7_) < 1e-6) {
                has_x7y2 = true;
            }
            if (std::abs(pose.pose.position.x - y1_) < 1e-6 && std::abs(pose.pose.position.y - x7_) < 1e-6) {
                has_x7y1 = true;
            }
            if (std::abs(pose.pose.position.x - y2_) < 1e-6 && std::abs(pose.pose.position.y - x1_) < 1e-6) {
                has_x1y2 = true;
            }
            if (std::abs(pose.pose.position.x - y1_) < 1e-6 && std::abs(pose.pose.position.y - x1_) < 1e-6) {
                has_x1y1 = true;
            }
        }
        // 경로 뒤에서 2번째 포즈의 y좌표 얻기
        second_to_last_x = global_path.poses[global_path.poses.size() - 2].pose.position.y;
        second_to_last_y = global_path.poses[global_path.poses.size() - 2].pose.position.x;
    ...
    nav_msgs::msg::Path CustomPlannerSimulation::createPlan(
        const geometry_msgs::msg::PoseStamped &start,
        const geometry_msgs::msg::PoseStamped &goal,
        std::function<bool()> cancel_checker)
    {
        ...
        bool has_start_wp = false;
        bool has_end_wp = false;
        ...
        // 3. 중간 웨이포인트 좌표 찾기 및 경로에 추가
        if (has_start_wp && has_end_wp) {
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
            std::string startKey =
            ...
            auto betweenWaypoints = GetBetweenWaypoints(start, goal, startKey, endKey);
            if (betweenWaypoints != std::nullopt && !betweenWaypoints->empty()) {
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
                MakePathByBetweenWaypoints(*betweenWaypoints, startWaypointIndex, endWaypointIndex, start);
            } else {
                RCLCPP_INFO(logger_, "중간 웨이포인트가 없어 직선 경로를 사용합니다.");
            }
        } else {
            RCLCPP_WARN(logger_, "웨이포인트 정보를 찾지 못해 start-goal 직선 경로를 사용합니다.");
        }
...
// 플러그인 등록
PLUGINLIB_EXPORT_CLASS(custom_planner_simulation::CustomPlannerSimulation, nav2_core::GlobalPlanner)

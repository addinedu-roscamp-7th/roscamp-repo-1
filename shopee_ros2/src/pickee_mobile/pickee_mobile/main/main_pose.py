...
class GetAmclPose(Node):
    """
    AMCL로부터 PoseWithCovarianceStamped 메시지를 구독하고
    x, y, yaw(θ)을 계산하여 로그로 출력합니다.
    """

    def __init__(self):
        super().__init__('amcl_pose_listener')
        self.get_logger().info('📡 AMCL Pose Listener 초기화 중...')
        ...
        self.get_logger().info('✅ /amcl_pose 구독 시작')

    def get_current_pose_callback(self, msg: PoseWithCovarianceStamped):
        """AMCL Pose 메시지를 기반으로 로봇의 현재 위치 계산"""
        x = msg.pose.pose.position.x
        y = msg.pose.pose.position.y

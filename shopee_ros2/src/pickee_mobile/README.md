...
module 폴더 main 폴더의 노드가 import 하는 노드혹은 코드들이 있는 폴더.  
test 폴더 : 기능구현을 연습하면서 사용했던 테스트 코드가 있는 폴더. mock_test 폴더에는 통신 테스트를 위한 코드, goal_test 폴더는 목적지 지정 및 위치 토픽 구독을 테스트한 코드, topic_test 폴더에는 속도제어, 위치 토픽 발행 테스트 코드가 있다.  
...
### 2.3 Aruco marker 도킹 ver_1
nav2로 목적지까지 주행을 완료하면 ArUco 마커를 탐지해서 근접 주차를 한다. 카메라 기준 마커의 좌표를 사용해서 접근한다.

```bash
ros2 launch pickee_mobile mobile_bringup.launch.xml # 로봇
ros2 launch pickee_mobile nav2_bringup_launch.xml # 로봇 맵 설정은 원하는대로
ros2 launch pickee_mobile nav2_view.launch.xml #pc 안해도 됨
...
### 2.4 Aruco marker 도킹 ver_2
위의 ver_1에서 x축 정렬 단계가 추가됐다. 카메라 기준 마커의 좌표를 변환해서 마커 기준 로봇의 좌표를 활용해서 접근한다. ver_1에 비해 작업시간은 감소했고 정확도는 상승했다.   
[이전 좌표계](https://github.com/lim123123/img/blob/main/pink/docking/%EB%A7%A4%EB%8C%80%EB%8F%84%EC%B0%A9_%EC%B9%B4%EB%A9%94%EB%9D%BC%EC%A2%8C%ED%91%9C%EA%B3%84_pickee.drawio.png)  
[이후 좌표계](https://github.com/lim123123/img/blob/main/pink/docking/%EB%A7%A4%EB%8C%80%EB%8F%84%EC%B0%A9_%EB%A7%88%EC%BB%A4%EC%A2%8C%ED%91%9C%EA%B3%84_pickee.drawio.png)  
[순서도](https://github.com/lim123123/img/blob/main/pink/docking/%EB%8F%84%ED%82%B9_ver_2_%EC%88%9C%EC%84%9C%EB%8F%84.png)  

```bash
ros2 launch pickee_mobile mobile_bringup.launch.xml # 로봇
ros2 launch pickee_mobile nav2_bringup_launch.xml # 로봇 맵 설정은 원하는대로
ros2 launch pickee_mobile nav2_view.launch.xml #pc 안해도 됨
ros2 launch pickee_mobile mobile_controller_once # pc
ros2 run pickee_mobile main_aruco_pub_once # pc

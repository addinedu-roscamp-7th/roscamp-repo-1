#!/usr/bin/env python3
"""
Mock Robot Node - 로봇 시뮬레이터

실제 로봇 없이 Main Service를 테스트하기 위한 Mock 로봇 노드입니다.
Pickee와 Packee의 동작을 시뮬레이션합니다.
"""
import sys
import logging
import random
...
class MockRobotNode(Node):
    ...
    Main Service의 모든 ROS2 인터페이스를 구현하여
    실제 로봇 없이도 전체 플로우를 테스트할 수 있습니다.
    """
    ...
    def handle_move_to_section(self, request, response):
        ...
        move_msg.destination = f"SECTION_{request.section_id}"
        self.pickee_move_pub.publish(move_msg)

        # 0.5초 후 도착 알림 (타이머로 비동기 처리)
        def publish_arrival():
        ...
        self.create_timer(0.5, publish_arrival, clock=self.get_clock())
    ...
    def handle_product_detect(self, request, response):
        ...
        self.create_timer(0.3, publish_detection, clock=self.get_clock())
    ...
    def handle_process_selection(self, request, response):
        ...
        self.create_timer(0.3, publish_selection, clock=self.get_clock())
    ...
    def handle_move_to_packaging(self, request, response):
        ...
        move_msg.robot_id = request.robot_id
        move_msg.order_id = request.order_id
        move_msg.destination = "PACKING_AREA"
        self.pickee_move_pub.publish(move_msg)
        ...
        self.create_timer(0.5, publish_handover, clock=self.get_clock())
    ...
    def handle_check_availability(self, request, response):
        ...
        response.robot_id = 10  # Mock Packee ID
    ...
    def handle_packing_start(self, request, response):
        ...
        self.create_timer(1.0, publish_packing_complete, clock=self.get_clock())
...
def main(args=None):
    ...
    try:
        logger.info("Mock Robot Node running... Press Ctrl+C to exit")
        rclpy.spin(node)

#!/usr/bin/env python3
"""
SC_05_3 - 작업 이력 조회 시나리오 실행 스크립트
"""
import argparse
import asyncio
import json

from shopee_main_service.scenario_suite import run_sc_05_3_robot_history_search


def parse_args() -> argparse.Namespace:
    """명령행 인자를 파싱합니다."""
    parser = argparse.ArgumentParser(description='SC_05_3 작업 이력 조회 시나리오 실행기')
    ...
    parser.add_argument('--filters', default='{}', help='검색 필터 JSON 문자열 (기본값: {})')
...
def main() -> None:
    ...
    asyncio.run(
        run_sc_05_3_robot_history_search(
            host=args.host,
            port=args.port,
            filters=filters,
        )
